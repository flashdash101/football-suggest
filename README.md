...
##  Core Algorithm
The recommendation logic is powered by the `AdvancedPlayerRecommender` class. This Python module implements a hybrid unsupervised learning and weighted scoring system with controlled randomness to surface player insights.
...
*   **Stochastic Ranking:** Uses controlled randomness to diversify the shortlist while keeping the highest-fit players near the top.
*   **Weighted Scoring:** Calculates a composite score using role-specific weights and style bonuses, then ranks players accordingly.

[ **View the Algorithm Source Code**](src/Recommend.py)
...
*   **Composite Scoring Engine:** Recommendations are generated via a weighted scoring system combining:
    *   Role-specific feature weighting (0.5–1.1×).
    *   Additive style bonuses.
    *   Percentile-rank normalisation.
    *   Controlled randomness to balance recommendation quality with result diversity.
...
4.  **Algorithmic Scoring:** The backend calculates a composite score based on the weighted importance of specific stats for your chosen role, then adds a small random variation to diversify the shortlist.
5.  **Recommendation:** The system returns a list of players that statistically match your criteria, with fast response times and a slightly different shortlist on refresh.
